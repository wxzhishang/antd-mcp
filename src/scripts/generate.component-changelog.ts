...
const output = 'docs/preset';

// Collect components
const componentNames = globSync(
  path
    .join(process.cwd(), 'ant-design/components/!(version|icon|col|row)/index.zh-CN.md')
    .split(path.sep)
    .join('/'),
)
...
export function generateComponentChangelog() {
  ...
  syncChangelog('ant-design/CHANGELOG.zh-CN.md', 'components-changelog-cn.json');
  syncChangelog('ant-design/CHANGELOG.en-US.md', 'components-changelog-en.json');
  fs.writeFileSync(
