...
2. **Ant Design Vue Transfer 穿梭框“造轮子”** (相关性: 30%)
   - 链接: https://juejin.cn/post/7001330858858807332
   - 摘要: 不可能一套组件满足所有的需求，前端开发总避免不了要“造轮子”的。这次做的穿梭框就是模仿官方来写的，功能基本一样，但不可能完全一样，会在官方的基础上优化，样式也更好看了哟。

3. **Ant Design Transfer 双树穿梭框“造轮子”** (相关性: 30%)
   - 链接: https://juejin.cn/post/7007682956584026119
   - 摘要: 主要实现完整的树形穿梭框，穿梭框左侧和右侧都是树形结构，而且父子节点有联动关系，还是用于城市选择组件
...
19. **使用ant-design-vue的upload进行文件或者文件夹的拖拽上传** (相关性: 30%)
   - 链接: https://juejin.cn/post/7473792518909689865
   - 摘要: 使用场景 系统中上传文件和文件夹的情况数不胜数，但是这次我们需要加上拖拽文件和文件夹的上传功能，目前文件和文件夹的拖拽需要用户手动选择，如图： 如果已经有上传好的文件，则还会增加这样的点击上传入口 实
...
基于收集到的信息，建议在使用 Transfer 组件时注意以下几点：
